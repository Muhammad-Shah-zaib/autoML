...
Automatically detects data quality issues and suggests fixes
"""
...
def detect_constant_features(df, target_column):
    ...
    for col in df.columns:
        ...
        # For all features, check unique value ratio
...
def display_issues_and_get_approval(issues, df):
    ...
    if len(issues['missing_values']) > 0:
        with st.expander("📊 Missing Values", expanded=True):
            ...
            **Options:**
            ...
            # Get user approval
            if st.checkbox("✅ I want to fix missing values", key="fix_missing"):
                ...
                st.session_state.preprocessing_decisions['fix_missing_values'] = False
    ...
    if len(issues['high_cardinality']) > 0:
        with st.expander("🔢 High Cardinality Features", expanded=True):
            ...
            **Options:**
            ...
            if st.checkbox("✅ Group rare categories", key="fix_cardinality"):
    ...
    if len(issues['constant_features']) > 0:
        with st.expander("📏 Constant/Near-Constant Features", expanded=True):
            ...
            if st.checkbox("✅ Remove constant features", key="remove_constant"):
    ...
    if len(st.session_state.preprocessing_decisions) > 0:
        decisions_df = pd.DataFrame([
            {'Decision': k, 'Action': str(v)}  # Convert to string to avoid Arrow warnings
